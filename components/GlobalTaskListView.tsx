...
const GlobalTaskListView: React.FC = () => {
    ...
    const toggleListPersistence = () => {
        ...
        if (!newValue) localStorage.removeItem(`taskbox-global-view-state-${viewId}`);
        else localStorage.setItem(`taskbox-global-view-state-${viewId}`, JSON.stringify({ sort, showCompleted, dependencyMode, excludedListIds: Array.from(excludedListIds) }));
    };

    const filterMenuRef = React.useRef<HTMLDivElement>(null);
    useEffect(() => {
        const handleClick = (e: MouseEvent) => {
            if (filterMenuRef.current && !filterMenuRef.current.contains(e.target as Node)) setShowListFilter(false);
        };
    ...
    const groupTasksByParent = (tasks: { task: Task; listTitle: string }[]) => {
        const childrenMap = new Map<number, { task: Task; listTitle: string }[]>();
        tasks.forEach(item => {
            if (item.task.parentTaskId) {
                if (!childrenMap.has(item.task.parentTaskId)) childrenMap.set(item.task.parentTaskId, []);
                childrenMap.get(item.task.parentTaskId)!.push(item);
            }
        });

        const currentIds = new Set(tasks.map(t => t.task.id));
        const roots = tasks.filter(item => !item.task.parentTaskId || !currentIds.has(item.task.parentTaskId));
        const result: { task: Task; listTitle: string; depth: number }[] = [];
        const processed = new Set<number>();

        const flatten = (item: { task: Task; listTitle: string }, depth: number) => {
            if (processed.has(item.task.id)) return;
            processed.add(item.task.id);
            result.push({ ...item, depth });
            const children = childrenMap.get(item.task.id) || [];
    ...
    const handleDrop = async (e: React.DragEvent<HTMLDivElement>, targetTask: Task) => {
        ...
        const updates = sortableList.map((item, index) => ({ id: item.task.id, globalSortOrder: index + 1 }));
        await reorderGlobalTasks(updates);
        setDraggedTask(null);
    };

    const sortedAndFilteredResults = useMemo(() => {
        // Step 1: Filter raw tasks based on core criteria (Completed, Search)
        let availableTasks = rawTasks.filter(({ task }) => {
            if (!showCompleted && task.completed) return false;
            if (localSearch && !task.description.toLowerCase().includes(localSearch.toLowerCase())) return false;
            return true;
        });

        // Step 2: Apply Special View Logic
        // In Dependency Mode (1 or 2), if a task matches the filter, we want to show its FAMILY too.
        
        let matchingTasks = availableTasks.filter(({ task }) => {
            if (specialView === 'all') return true;
            if (specialView === 'importance') return task.importance >= 1;
            if (specialView === 'pinned') return task.pinned;
            if (specialView === 'dependencies') return !!task.parentTaskId || availableTasks.some(t => t.task.parentTaskId === task.id);
            if (specialView === 'focused') return task.focused;
            if (specialView === 'due_tasks') return !!task.dueDate;
            return true;
        });

        // Step 3: Family Expansion if Grouping is Enabled
        let finalTasks = matchingTasks;
        
        if (dependencyMode > 0) {
            const tasksToInclude = new Set<number>(matchingTasks.map(t => t.task.id));
            ...
            availableTasks.forEach(item => {
                ...
                if (item.task.parentTaskId) {
                    if (!childrenMap.has(item.task.parentTaskId)) childrenMap.set(item.task.parentTaskId, []);
                    childrenMap.get(item.task.parentTaskId)!.push(item.task.id);
                }
            });

            // Expand
            // For each matching task, traverse UP to root and DOWN to leaves
            const expand = (id: number) => {
                ...
                // Traverse Up
                if (item.task.parentTaskId && !tasksToInclude.has(item.task.parentTaskId)) {
                    expand(item.task.parentTaskId);
                }
                
                // Traverse Down (Children)
                const children = childrenMap.get(id) || [];
                children.forEach(childId => {
                    if (!tasksToInclude.has(childId)) {
                        tasksToInclude.add(childId);
                        const grandChildren = childrenMap.get(childId) || [];
                        grandChildren.forEach(gc => tasksToInclude.add(gc));
                    }
                });
            };

            matchingTasks.forEach(t => expand(t.task.id));
            finalTasks = availableTasks.filter(t => tasksToInclude.has(t.task.id));
        }

        // Calculate parent IDs from the current filtered set for efficient sorting lookup
        const parentIds = new Set(finalTasks.map(item => item.task.parentTaskId).filter((id): id is number => id !== null && id !== undefined));

        // Step 4: Sort
        const compareValues = (a: any, b: any, type: 'string' | 'number' | 'date' | 'boolean', direction: 'asc' | 'desc') => {
            let valA = a ?? ''; let valB = b ?? '';
            if (type === 'string') {
                valA = String(valA).toLowerCase(); valB = String(valB).toLowerCase();
                return direction === 'asc' ? valA.localeCompare(valB) : valB.localeCompare(valA);
            }
            if (type === 'date') return direction === 'asc' ? String(valA).localeCompare(String(valB)) : String(valB).localeCompare(String(valA));
        ...
        const sortTasks = (taskList: typeof finalTasks) => {
            if (isCustomSortEnabled) return taskList.sort((a, b) => (a.task.globalSortOrder || 0) - (b.task.globalSortOrder || 0));
            const isDependencySort = dependencyMode > 0;
            return taskList.sort((aItem, bItem) => {
                const a = aItem.task; const b = bItem.task;
                
                if (isDependencySort) {
                    const aDep = !!a.parentTaskId; const bDep = !!b.parentTaskId;
        ...
        const sortedList = sortTasks(finalTasks);
        if (dependencyMode === 0 || isCustomSortEnabled) return sortedList.map(item => ({ ...item, depth: 0 }));
        
        let grouped = groupTasksByParent(sortedList);
        
        if (dependencyMode === 2) {
             const parents = new Set(sortedList.filter(i => i.task.parentTaskId).map(i => i.task.parentTaskId));
             const hasChildren = (id: number) => parents.has(id);
             grouped = grouped.filter(item => !!item.task.parentTaskId || hasChildren(item.task.id));
        }
        return grouped;
    }, [rawTasks, showCompleted, localSearch, sort, dependencyMode, isCustomSortEnabled, specialView]);
    ...
    const toggleListExclusion = (id: number) => setExcludedListIds(prev => { const next = new Set(prev); if (next.has(id)) next.delete(id); else next.add(id); return next; });
    const toggleSelectAll = () => setExcludedListIds(excludedListIds.size > 0 ? new Set() : new Set(lists.map(l => l.id)));
        ...
        <div className="p-4 sm:p-6 flex flex-col h-full relative print:block print:h-auto print:overflow-visible">
            <div className="mb-6 flex justify-between items-center">
                 <div className="w-full">
                    ...
                    <p className="text-gray-500 mt-2 print-hidden">
                        {specialView === 'focused' ? 'Your active focus list.' : specialView === 'due_tasks' ? 'Tasks with due dates.' : 'Showing active tasks from all lists.'}
                    </p>
                 </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 mb-4 items-center flex-wrap no-print relative z-10">
                <div className="relative flex-grow max-w-xs">
                    <input type="text" placeholder="Filter current view..." value={localSearch} onChange={(e) => setLocalSearch(e.target.value)} className={`block w-full pl-8 pr-10 h-10 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 ${inputBg}`} />
                ...
                <div className="flex items-center space-x-2 flex-grow sm:flex-grow-0 sm:ml-auto">
                    ...
                    {canUseCustomSort && (
                        <Tooltip text={isCustomSortEnabled ? "Disable Custom Order" : "Enable Custom Order"} debugLabel="Global Custom Sort Toggle">
                            <button onClick={() => setIsCustomSortEnabled(!isCustomSortEnabled)} className={`flex items-center justify-center h-10 w-10 rounded-md border transition-colors ${isCustomSortEnabled ? (isOrange ? 'bg-orange-900/50 border-orange-500 text-orange-400' : 'bg-blue-100 border-blue-400 text-blue-600 dark:bg-blue-900/40 dark:border-blue-500 dark:text-blue-400') : (isOrange ? 'border-gray-600 hover:bg-gray-800 text-gray-500' : 'border-gray-300 hover:bg-gray-100 dark:border-gray-600 dark:hover:bg-gray-700 text-gray-500')}`}>
                    ...
                    <div className="relative" ref={filterMenuRef}>
                        <Tooltip text="Filter List">
                            <button onClick={() => setShowListFilter(!showListFilter)} className={`flex items-center justify-center h-10 w-10 rounded-md border transition-colors ${isOrange ? 'border-gray-600 hover:bg-gray-800 text-gray-400' : 'border-gray-300 hover:bg-gray-100 dark:border-gray-600 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300'}`}>
                        ...
                        {showListFilter && (
                            <div className={`absolute right-0 mt-2 w-64 rounded-md shadow-lg py-2 z-50 max-h-80 overflow-y-auto ${isOrange ? 'bg-gray-800 border border-gray-700' : 'bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600'}`}>
                                <div className={`flex justify-between items-center px-4 py-2 border-b ${isOrange ? 'border-gray-700' : 'border-gray-200 dark:border-gray-600'}`}>
                                    ...
                                    <button onClick={toggleSelectAll} className={`hover:text-blue-500 transition-colors ${isOrange ? 'text-gray-400' : 'text-gray-500'}`} title={isAllSelected ? "Deselect All" : "Select All"}>
                                        {isAllSelected ? <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-blue-500" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg> : <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
                                    </button>
                                </div>
                                {lists.map(l => (
                                    <label key={l.id} className={`flex items-center px-4 py-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 ${isOrange ? 'hover:bg-gray-700' : ''}`}>
                                        <input type="checkbox" checked={!excludedListIds.has(l.id)} onChange={() => toggleListExclusion(l.id)} className="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" />
                    ...
                    {specialView !== 'dependencies' && !isCustomSortEnabled && (
                        <Tooltip text={getDependencyToggleText()} debugLabel="Global Dependency Toggle">
                            <button onClick={() => setDependencyMode((prev) => (prev + 1) % 3)} className={`flex items-center justify-center h-10 w-10 rounded-md border transition-colors ${dependencyMode > 0 ? (isOrange ? 'bg-orange-900/50 border-orange-500' : 'bg-blue-100 border-blue-400 dark:bg-blue-900/40 dark:border-blue-500') : (isOrange ? 'border-gray-600 hover:bg-gray-800' : 'border-gray-300 hover:bg-gray-100 dark:border-gray-600 dark:hover:bg-gray-700')}`}>
                    ...
                    <Tooltip text={showCompleted ? "Hide Completed tasks" : "Show Completed tasks"} debugLabel="Global Completed Toggle">
                        <button onClick={() => setShowCompleted(!showCompleted)} className={`flex items-center justify-center h-10 w-10 rounded-md border transition-colors ${showCompleted ? (isOrange ? 'bg-orange-900/50 border-orange-500 text-orange-400' : 'bg-blue-100 border-blue-400 text-blue-600 dark:bg-blue-900/40 dark:border-blue-500 dark:text-blue-400') : (isOrange ? 'border-gray-600 hover:bg-gray-800 text-gray-500' : 'border-gray-300 hover:bg-gray-100 dark:border-gray-600 dark:hover:bg-gray-700 text-gray-500')}`}>
            ...
            <div className="space-y-4 flex-grow overflow-y-auto no-scrollbar pb-4 print:overflow-visible print:h-auto">
                {sortedAndFilteredResults.length > 0 ? (
                    sortedAndFilteredResults.map(({ task, listTitle, depth }) => {
                        return (
                            <div key={task.id} className="relative" style={{ marginLeft: `${depth * 1}rem` }}>
                    ...
                    <div className="text-center py-16 text-gray-500">
                        {specialView === 'focused' ? 'No tasks focused.' : 'No active tasks found matching criteria.'}
                    </div>
                )}
            </div>
        </div>

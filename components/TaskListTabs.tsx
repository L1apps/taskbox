...
const TaskListTabs: React.FC<TaskListTabsProps> = ({ lists, activeListId, onSelectList, theme }) => {
  ...
  return (
    <div className="h-full overflow-y-auto no-scrollbar py-2 pl-2">
        ...
        {lists.map(list => {
            ...
            return (
              <div 
                key={list.id} 
                className={`
                    flex items-center justify-between px-4 py-3 rounded-l-lg cursor-pointer transition-colors duration-200 select-none

...
import type { Theme, User, TaskListWithUsers } from '../types';
...
const Header: React.FC<HeaderProps> = ({ theme, onToggleTheme, onImport, onPaste, onExport, onShowAbout, onShowStats, onShowAdminPanel, onShowUserSettings, user, onLogout, activeList }) => {
  ...
  // Check if import/export should be disabled (if list has sublists)
  const isContainer = activeList && activeList.children && activeList.children.length > 0;
  const isImportExportDisabled = !activeList || isContainer;
        ...
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-baseline space-x-2">
                ...
                <span className={`text-xs font-mono ${isOrange ? 'text-gray-500' : 'text-gray-400'}`}>v{APP_VERSION}</span>
            </div>
          </div>
          <div className="flex items-center space-x-1 sm:space-x-2">
            {user ? (
              <>
                <Tooltip text={isContainer ? "Disabled for Container Lists" : "Paste tasks"}>
                  <button onClick={onPaste} disabled={isImportExportDisabled} className={`flex ${iconButtonBase} ${isOrange ? orangeHover : defaultHover} ${isImportExportDisabled ? disabledClass : ''}`}>
                ...
                <Tooltip text={isContainer ? "Disabled for Container Lists" : "Import from file"}>
                  <button onClick={onImport} disabled={isImportExportDisabled} className={`hidden sm:flex ${buttonBase} ${isOrange ? orangeHover : defaultHover} ${isImportExportDisabled ? disabledClass : ''}`}>
                     <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
                     <span className="hidden md:inline">Import</span>
                  </button>
                </Tooltip>
                <Tooltip text={isContainer ? "Disabled for Container Lists" : "Export tasks"}>
                  <button onClick={onExport} disabled={isImportExportDisabled} className={`hidden sm:flex ${buttonBase} ${isOrange ? orangeHover : defaultHover} ${isImportExportDisabled ? disabledClass : ''}`}>
                ...
                <Tooltip text="View on GitHub">
                    <a href="https://github.com/L1apps/taskbox" target="_blank" rel="noopener noreferrer" aria-label="GitHub" className={`${iconButtonBase} ${isOrange ? orangeHover : defaultHover}`}>
                ...
                <div className="flex items-center space-x-2">
                    ...
                    {user.role === 'ADMIN' && (
                     ...
                     <Tooltip text="Logout">
                        <button onClick={onLogout} aria-label="Logout" className={`${iconButtonBase} ${isOrange ? orangeHover : defaultHover}`}>
